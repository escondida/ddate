...
thud:
	if (argc - pi == 3) {
		int moe = atoi(argv[pi]), larry = atoi(argv[pi + 1]),
			curly = atoi(argv[pi + 2]);
		hastur = makeday(
#ifdef US_FORMAT
				moe, larry,
#else
		...
		if ((i == tib_start) && (dt.day == -1)) {
			...
			if (fmt[i] == '%') {
				...
				case 'b':
					...
					break;
				case 'd':
					sprintf(snarf, "%d", dt.day + 1);
					wibble = snarf;
					break;
				case 'e':
					sprintf(snarf, "%d%s", dt.day + 1, ending(dt.day + 1));
					wibble = snarf;
					break;
				case 'H':
...
struct disc_time
makeday(int imonth, int iday, int iyear) /*i for input */
{
